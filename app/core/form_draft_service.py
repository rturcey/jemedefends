...
class FormDraftService:
    ...
    async def autosave(
        ...
        logger.info(f"Draft autosave - existing: {len(existing_data)} fields, new: {len(data)} fields, merged: {len(merged_data)} fields")
        logger.info(f"Merged data keys: {list(merged_data.keys())}")

        await self.repo.update_data(draft_id=draft.id, data=merged_data, last_event="autosave")
    ...
    async def mark_submitted_and_clear_cookie(
        ...
        # Le cookie est normalement géré par le repo lors de la création.
        # Ici on le supprime pour clôturer le cycle.
