...
dev-front:
	@echo "🎨  Tailwind watch + 🚀 FastAPI (reload) — démarrage…"
	@set -e; \
	npm run tw:watch & CSS_PID=$$!; \
	echo "👉 Tailwind watch PID: $$CSS_PID"; \
	trap 'echo "\n🛑 Stop…"; kill $$CSS_PID 2>/dev/null || true' EXIT INT TERM; \
	uv run uvicorn app.main:app --reload --host 0.0.0.0 --port 8000

dev-front-debug:
	@echo "🎨  Tailwind watch + 🚀 FastAPI (debug) — démarrage…"
	@set -e; \
	npm run tw:watch & CSS_PID=$$!; \
	echo "👉 Tailwind watch PID: $$CSS_PID"; \
	trap 'echo "\n🛑 Stop…"; kill $$CSS_PID 2>/dev/null || true' EXIT INT TERM; \
