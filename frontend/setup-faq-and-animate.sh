...
mkdir -p src/components/faq src/app/faq

echo "==> Writing src/components/faq/FaqSectionSticky.tsx"
cat > src/components/faq/FaqSectionSticky.tsx <<'TSX'
...
import {
  ArrowRight, HelpCircle, Shield, FileText, FileWarning, Lock, Info, ChevronDown,
  CheckCircle2, Mail, FileCheck2, Sparkles,
} from 'lucide-react';
  ...
  return (
    <details className="group rounded-xl border border-gray-200 bg-white p-3 hover:shadow-sm transition-shadow open:ring-1 open:ring-blue-500/15">
      ...
      <div className="mt-2 text-sm leading-6 text-gray-700">
        ...
        {href && (
          <div className="mt-2">
            <Link href={href} className="text-blue-600 hover:underline">👉 Approfondir dans le guide</Link>
          </div>
        )}
      </div>
    </details>
...
function CategoryBar({
  current, onChange,
}: { current: CatId | 'all'; onChange: (id: CatId | 'all') => void; }) {
  const cats = [{ id: 'all', label: 'Tout', icon: <Info className="w-4 h-4" /> } as const]
    .concat(FAQ_CATEGORIES.map(({ id, label, icon }) => ({ id, label, icon })));
  return (
    ...
    <aside className="lg:sticky lg:top-24 lg:self-start">
      <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
        ...
        <p className="mt-2 text-sm text-gray-700">
          Nous fournissons un <strong>outil</strong> pour enclencher vos droits et créer une <strong>preuve</strong>.
          Nous ne représentons pas les consommateurs, mais on vous aide à garder un dossier <strong>propre</strong>.
        </p>

        <ul className="mt-4 space-y-2 text-sm">
          <li className="flex items-start gap-2"><FileCheck2 className="h-4 w-4 text-blue-700 mt-0.5" /><span>Lettre fondée sur le Code de la conso</span></li>
          <li className="flex items-start gap-2"><Mail className="h-4 w-4 text-blue-700 mt-0.5" /><span>Recommandé conseillé (AR)</span></li>
          <li className="flex items-start gap-2"><CheckCircle2 className="h-4 w-4 text-green-700 mt-0.5" /><span>Gratuit : contenu complet</span></li>
        </ul>

        <div className="mt-4 grid gap-2">
          <Button href="/eligibilite" icon={<ArrowRight className="w-4 h-4" />}>Générer ma lettre gratuitement</Button>
          <Button href="/eligibilite" variant="secondary" icon={<Sparkles className="w-4 h-4" />}>Voir les options premium</Button>
        </div>

        <p className="mt-3 text-xs text-gray-500">
          Transparence&nbsp;: pas de promesse de résultat. Si le litige persiste, pensez à SignalConso, au conciliateur de justice ou à une association.
        </p>
      </div>
    </aside>
  );
...
}: { compact?: boolean; emitSchema?: boolean }) {
      ...
      <Container>
        ...
        <div className={['grid lg:grid-cols-3', compact ? 'gap-4' : 'gap-6'].join(' ')}>
          <div className="lg:col-span-2">
            ...
            <Reveal>
              <div className={['grid grid-cols-1 md:grid-cols-2', compact ? 'gap-3 lg:gap-4' : 'gap-4 lg:gap-5'].join(' ')}>
                {visible.map(category => (
                  <div key={category.id} className="space-y-3">
                    {cat !== 'all' && (
                      <h3 className="text-sm font-bold uppercase tracking-wide text-gray-600">{category.label}</h3>
                    )}
...
echo "==> Writing src/app/faq/page.tsx"
cat > src/app/faq/page.tsx <<'TSX'
// src/app/faq/page.tsx
import type { Metadata } from 'next';
import FaqSectionSticky from '@/components/faq/FaqSectionSticky';

export const metadata: Metadata = {
  title: 'FAQ - Je me défends',
  description: 'Réponses rapides et fiables pour faire valoir vos droits de consommateur.',
...
export default function FaqPage() {
  return (
    <main>
      {/* IMPORTANT : schema JSON-LD activé uniquement ici */}
      <FaqSectionSticky compact={false} emitSchema />
    </main>
  );
}
TSX

echo "==> Removing any import of 'tw-animate-css' and switching to tailwindcss-animate"
# 1) Remove tw-animate-css imports in CSS or TS files (globals.css most likely)
if ls src/**/*.css 1> /dev/null 2>&1; then
  grep -rl "tw-animate-css" src | while read -r f; do
    echo "   - Stripping 'tw-animate-css' from $f"
    sed -i '/tw-animate-css/d' "$f"
  done
fi
if [ -f src/app/globals.css ]; then
  sed -i '/tw-animate-css/d' src/app/globals.css || true
fi

# 2) Install tailwindcss-animate (dev dep)
if [ -f package.json ]; then
  echo "==> Installing tailwindcss-animate…"
  npm i -D tailwindcss-animate >/dev/null 2>&1 || yarn add -D tailwindcss-animate >/dev/null 2>&1 || pnpm add -D tailwindcss-animate >/dev/null 2>&1 || true
else
  echo "!! package.json introuvable, installe manuellement: npm i -D tailwindcss-animate"
fi

# 3) Ensure tailwind.config.js has plugin + safelist
if [ -f tailwind.config.js ]; then
  if ! grep -q "tailwindcss-animate" tailwind.config.js; then
    echo "==> Adding tailwindcss-animate plugin to tailwind.config.js"
    # Insert plugin before the closing brace of module.exports
    tmp=$(mktemp)
    awk '
      /module\.exports\s*=\s*{/{print; seen=1; next}
      seen && /plugins:\s*\[/ {print; print "    require(\"tailwindcss-animate\"),"; seen=2; next}
      seen==1 && /}$/ {print "  ,plugins: [require(\"tailwindcss-animate\")]"; print; seen=0; next}
      {print}
    ' tailwind.config.js > "$tmp" && mv "$tmp" tailwind.config.js
  fi

  if ! grep -q "(bg|text|border)-(blue|purple|green|indigo|orange|pink|yellow|gray)-(50|100|200|600|700)" tailwind.config.js; then
    echo "==> Adding safelist for dynamic category colors"
    tmp=$(mktemp)
    awk '
      /module\.exports\s*=\s*{/{print; print "  safelist: [\n    { pattern: /(bg|text|border)-(blue|purple|green|indigo|orange|pink|yellow|gray)-(50|100|200|600|700)/ },\n  ],"; next}
      {print}
    ' tailwind.config.js > "$tmp" && mv "$tmp" tailwind.config.js
  fi
else
  echo "!! tailwind.config.js introuvable - ajoute manuellement le plugin et la safelist."
fi

echo "==> All done."
echo "=> Redémarre proprement : rm -rf .next && npm run dev"
