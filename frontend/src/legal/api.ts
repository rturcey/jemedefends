...
class LegalAPI {
    ...
    private async _loadRegistryInternal(): Promise<void> {
        try {
            ...
            console.warn('Registry non disponible, mode dégradé:', error);
     ...
     * Registry doit être déjà chargé
     */
    getArticleSync(
        articleId: LegalArticleId,
        options: GetArticleOptions = {},
    ...
    private _validateArticleText(
        ...
        if (status === 'ABROGE') {
            ...
            if (!kw.some(k => text.toLowerCase().includes(k))) {
                console.warn(`Article ${articleId} marqué abrogé mais le texte ne le confirme pas`);
            }
        }
    }
