  ...
  return (
    <div className="py-6 md:py-12 lg:py-16 border-t border-gray-100">
      <div className="max-w-6xl mx-auto px-4">
        ...
        <div className="grid gap-2 sm:gap-3 md:gap-4 grid-cols-1 sm:grid-cols-2 items-start">
          {displayedItems.map((item, index) => (
            <Reveal key={item.id} delay={index * 0.05}>
              <div className="rounded-xl border border-gray-200 p-2 sm:p-3 md:p-4 hover:border-blue-300 hover:shadow-sm transition-all">
                <button
                ...
                >
                  <div className="flex items-start justify-between gap-2">
                    <div className="flex items-start gap-2 flex-1 min-w-0">
                      ...
                      <div className="flex h-8 w-8 sm:h-10 sm:w-10 items-center justify-center rounded-xl bg-blue-50 text-blue-600 flex-shrink-0">
                        {React.cloneElement(getContextualIcon(item.iconType, 'default'), {
                          className: 'w-4 h-4 sm:w-5 sm:h-5',
                          strokeWidth: 1.5,
                        })}
                      </div>
                      <span className="text-xs sm:text-sm md:text-base font-semibold text-gray-900 leading-snug">
                        {item.question}
                      </span>
                    </div>
