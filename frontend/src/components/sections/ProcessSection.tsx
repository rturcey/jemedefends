            ...
            <Container>
                    ...
                    <Reveal>
                        <div
                            ...
                            <p className="text-base md:text-lg text-slate-600 text-center">
                                Un processus simple et rapide pour faire valoir vos
                ...
                <div
                    ...
                    {STEPS.map((step, i) => {
                        ...
                        return (
                            <Skeleton key={step.title} loading={!isLoaded}
                                ...
                                <Reveal delay={i * 0.15}>
                                    <div className="relative text-center group">
                                        ...
                                        >
                                            <div
                                                className={colors.icon}>{step.icon}</div>
                                            <div
                                                className="absolute -top-2 -right-2 w-7 h-7 md:w-8 md:h-8 bg-slate-900 text-white text-sm font-bold rounded-full flex items-center justify-center">
                                                {i + 1}
                                            </div>
                                        </div>
                                        ...
                                        <div className="space-y-2 md:space-y-3">
                                            <div
                                                className="inline-flex items-center gap-2 px-3 py-1 bg-gray-100 rounded-full text-xs font-semibold text-gray-600">
                                                <div
                                                    className={`w-2 h-2 rounded-full ${colors.dot}`}/>
                                                {step.time}
                                            </div>
                                            <h3 className="font-bold text-lg md:text-xl text-slate-900">{step.title}</h3>
                                            <p className="text-sm md:text-base text-slate-600 leading-relaxed px-2">{step.desc}</p>
                                        </div>
                                    </div>
                                </Reveal>
                            </Skeleton>
                ...
                {/* CTA compact mobile */}
                <Skeleton loading={!isLoaded}
                          className="h-16 md:h-20 w-80 mx-auto mt-8 md:mt-12 lg:mt-16">
                    <Reveal delay={0.8}>
