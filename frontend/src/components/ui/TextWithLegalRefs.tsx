...
import { type LegalArticleId, isValidLegalArticleId } from '../registry';
...
export default function TextWithLegalRefs({
  ...
  // Préchargement optionnel des articles (pour UX)
  React.useEffect(() => {
    if (preloadArticles && foundArticles.length > 0) {
      // Trigger le chargement en arrière-plan
      foundArticles.forEach(articleId => {
        import('../api').then(({ legalAPI }) => {
          legalAPI.getArticle(articleId, { strict: false }).catch(() => {
            // Silent fail pour preload
          });
        });
      });
    }
  }, [foundArticles, preloadArticles]);
