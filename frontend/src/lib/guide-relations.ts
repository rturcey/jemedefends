...
// ============================================================================
// UTILITAIRES DE DEBUG ET STATS (développement)
// ============================================================================

/**
 * Version de debug qui retourne les scores détaillés
 */
export function buildRelatedGuidesWithScores(
  refSlug: string,
  guideRegistry: Record<string, string>,
  options: RelatedGuideOptions = {},
): GuideScore[] {
  const { minScore = 0.06, includeKeywords = true, includeLegalArticles = true } = options;
  ...
  try {
    ...
    console.error('Erreur buildRelatedGuidesWithScores:', error);
    return [];
  }
}

/**
 * Analyse la qualité des recommandations pour tous les guides
 */
export function analyzeRecommendationQuality(guideRegistry: Record<string, string>): {
  totalGuides: number;
  guidesWithRecommendations: number;
  averageRecommendations: number;
  categoryDistribution: Record<string, number>;
} {
  const stats = {
    totalGuides: 0,
    guidesWithRecommendations: 0,
  ...
  for (const slug of Object.keys(guideRegistry)) {
    stats.totalGuides++;

    const category = detectCategoryFromSlug(slug);
    stats.categoryDistribution[category] = (stats.categoryDistribution[category] || 0) + 1;

    const related = buildRelatedGuidesSmart(slug, guideRegistry, { limit: 4 });

    if (related.length > 0) {
      stats.guidesWithRecommendations++;
      totalRecommendations += related.length;
    }
  }

  stats.averageRecommendations = totalRecommendations / stats.totalGuides;
...
import { getAllGuides, GUIDE_YAML_REGISTRY } from '@/lib/guide-registry';

// Recommandations simples
const related = buildRelatedGuidesSmart(
  'smartphone-ecran-batterie-defaut-garantie-legale',
  GUIDE_YAML_REGISTRY,
  { limit: 4, minScore: 0.06, preferSameCategory: true }
);

console.log('Guides recommandés:', related);

// Debug avec scores
const withScores = buildRelatedGuidesWithScores(
  'smartphone-ecran-batterie-defaut-garantie-legale',
  GUIDE_YAML_REGISTRY,
  { minScore: 0.01 } // Score plus bas pour voir plus de candidats
);

console.log('Scores détaillés:');
withScores.forEach(score => {
  console.log(`${score.slug}: ${score.score.toFixed(3)} - ${score.reasons.join(', ')}`);
});

// Analyse de qualité
const qualityStats = analyzeRecommendationQuality(GUIDE_YAML_REGISTRY);
console.log('Statistiques de qualité:', qualityStats);
