      ...
      for (const rule of rules) {
        ...
        switch (rule.type) {
          ...
          case 'pattern':
            ...
            break;
          case 'number':
            // tu peux étendre ici plus tard si besoin
            isValid = value === '' || value === null || !Number.isNaN(Number(value));
            break;
        }

        if (!isValid) {
          const result: ValidationResult = { valid: false, message: rule.message };
          this.validationCache.set(cacheKey, result);
          return result;
        }
      }
...
/**
 * Props communes à tous les steps (BuyerInfoStep, SellerInfoStep, etc.)
 * utilisées dans FormGenerator via commonStepProps.
 */
export interface StepProps {
  ...
  /** Utilisé uniquement par ProblemInfoStep (dernier step) */
  onSubmit?: () => void;
}
